...
public class TheClockView extends View {
    ...
    public TheClockView(Context context) {
        super(context);
        init();
    }
    ...
    public TheClockView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float r = Math.min(getHeight() / 2, getWidth() / 2) - borderWidth / 2;//半径是长宽中小的一半

        Paint paintCircle = new Paint();

        paintCircle.setStyle(Paint.Style.STROKE);//设置填充样式为描边

        paintCircle.setAntiAlias(true);//抗锯齿功能

        paintCircle.setStrokeWidth(borderWidth);//设置画笔宽度

        canvas.drawCircle(getWidth() / 2, getHeight() / 2, r, paintCircle);//圆心在中心位置,
        ...
        for(int i=0;i<60;i++){
            ...
            if(i % 5 == 0){
                ...
                paintDegree.setStrokeWidth(6);//宽度

                degreeLength = DEFAULT_LONG_DEGREE_LENGTH;

            }else{

                paintDegree.setStrokeWidth(3);//宽度
...
//            canvas.rotate(360/60);//是围绕(0,0)旋转的
        ...
        paintHour.setColor(getResources().getColor(R.color.gold));
        ...
        paintMinute.setColor(getResources().getColor(R.color.silver));
        ...
        paintSecond.setColor(getResources().getColor(R.color.copper));
        ...
        canvas.drawCircle(0, 0, 3, paintCenter);//这时候的画布已经移动到了中心位置
